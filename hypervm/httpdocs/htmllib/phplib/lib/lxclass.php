...
abstract class Lxclass
{
	...
	function substr($var, $a, $b)
	{
		return substr($this->$var, $a, $b);
	}
	...
	static function isdefaultHardRefresh()
	{
		return false;
	}
	...
	final function getVirtualList($class, &$count, $sortby = null, $sortdir = null)
	{
		...
		if (!$ifdb) {
			...
			if ($sortby) {
				...
				if (!isset($this->$__tvar) || $this->__sortby_{
				$class} != $sortby || $this->__sortdir != $sortdir) {
...
*/
	...
	static function createAddformlist($parent, $class)
	{
		return false;
	}
	static function createListUpdateForm($parent, $class)
	{
		return null;
	}
