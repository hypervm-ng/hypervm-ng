...
class Servermail__Qmail  extends lxDriverClass
{
	...
	function save_xinetd_qmail()
	{
		...
		$maps = null;
		if ($this->main->isOn("enable_maps")) {
			$maps = "/usr/bin/rblsmtpd -r bl.spamcop.net";
		}

		$domkey = null;
		if ($this->main->isOn('domainkey_flag')) {
			$domkey = "DKSIGN=/var/qmail/control/domainkeys/%/private";
		}
		$virus = null;
		if ($this->main->isOn('virus_scan_flag')) {
			$virus = "QMAILQUEUE=/var/qmail/bin/simscan";
		}
		$spamdyke = null;
